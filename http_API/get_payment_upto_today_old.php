...
//    echo '<br>' . $total_balance;
    ...
    $sql = "SELECT SUM(received_amount) AS total_received_amount FROM `payment` WHERE `agent`='$agent' AND `insertion_date`>'$clear_date'";
    $result = $con->query($sql);
    $row = mysqli_fetch_assoc($result);
    if ($row['total_received_amount'] != '') {

        $total_balance = $total_balance - $row['total_received_amount'];
    }
