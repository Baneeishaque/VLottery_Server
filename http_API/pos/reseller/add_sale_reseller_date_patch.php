...
$bill_id_query = "SELECT MAX(bill_no) AS id,`reseller_configuration`.`status` AS `flavour_status` from ticket,`reseller_configuration`";
$bill_no_result = $con->query($bill_id_query);

$bill_no_row = mysqli_fetch_assoc($bill_no_result);
if ($bill_no_row['flavour_status'] == '1') {
    if ($bill_no_row['id'] == '') {
        $bill_no = 1;
    } else {
