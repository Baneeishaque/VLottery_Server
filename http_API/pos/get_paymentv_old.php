...
//            $emptyarray[] = $row;
        ...
        /* $sql = "SELECT (`prize_money`*`count`) AS `win_amount`,`ticket`.`id`, `bill_no`, `ticket`.`serial`, `count`, `agent`, `ticket`.`scheme`, `amount`, `insertion_date`,`insertion_time`,`prize_money` FROM `ticket`,`resultv` WHERE `resultv`.`serial`=`ticket`.`serial` AND `resultv`.`scheme`=`ticket`.`scheme` AND `ticket`.`insertion_date`=`resultv`.`date_time` AND `agent`='$agent' AND `insertion_date` BETWEEN '$start' AND '$end' ORDER BY `bill_no` DESC,`insertion_date`,`scheme`";
          //echo $sql;
          $result = $con->query($sql);
...
//            $emptyarray[] = $row;
        ...
        /* $sql = "SELECT (`prize_money`*`count`) AS `win_amount`,`ticket`.`id`, `bill_no`, `ticket`.`serial`, `count`, `agent`, `ticket`.`scheme`, `amount`, `insertion_date`,`insertion_time`,`prize_money` FROM `ticket`,`resultv` WHERE `resultv`.`serial`=`ticket`.`serial` AND `resultv`.`scheme`=`ticket`.`scheme` AND `ticket`.`insertion_date`=`resultv`.`date_time` AND `agent`='$agent' AND `insertion_date` BETWEEN '" . date_format($start_date, 'Y-m-d') . "' AND '" . date_format($end_date, 'Y-m-d') . "' ORDER BY `bill_no` DESC,`insertion_date`,`scheme`";
          //echo $sql;
          $result = $con->query($sql);
